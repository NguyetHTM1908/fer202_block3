...
const RecipeCard = ({ recipe, onViewRecipe, onAddToFavourite, isFavourite = false, index }) => {
  const styles = {
    recipeCard: {
      background: 'white',
    ...
    favouriteIcon: {
      fontSize: '1.25rem',
      color: isFavourite ? '#e74c3c' : '#6c757d',
      transition: 'all 0.3s ease'
    },
    favouriteBadge: {
      position: 'absolute',
  ...
  return (
    ...
    >
      <div style={styles.recipeImage}>
        ...
        >
          <span style={styles.favouriteIcon}>♡</span>
        </button>
        
        {/* Favourite Badge */}
        {isFavourite && (
          <div style={styles.favouriteBadge}>
            ♥
          </div>
        )}
      </div>
