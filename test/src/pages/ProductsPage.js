...
export default function ProductsPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await api.get('/products'); // qua proxy hoặc ENV
      const normalized = (data || []).map(p => ({
  id: p.id,
  name: p.title || p.name,
  image: assetUrl(p.image) || `https://picsum.photos/seed/${p.id}/600/400`,
  price: p.price,
  ...
  return (
    <Container className="py-4">
      ...
      {filteredProducts.length === 0 && (
        <Row className="mt-4">
          <Col>
            <Card className="text-center"><Card.Body>No results</Card.Body></Card>
          </Col>
        </Row>
      )}
    </Container>
